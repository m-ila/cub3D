...
/*   Updated: 2024/05/12 16:19:58 by mbruyant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_hor_or_ver(t_segment *seg)
{
	t_point_d	d;
	t_point_d	end;
    double	length;

	d.x = cos(ft_deg_to_rad(seg->angle));
	d.y = sin(ft_deg_to_rad(seg->angle));
	length = sqrt(d.x * d.x + d.y * d.y);
    d.x /= length;
    d.y /= length;
	end.x = (double) seg->until.x;
	end.y = (double) seg->until.y;
	end.x -= d.x / TILE_SIZE;
	end.y += d.y / TILE_SIZE;
	if ((int) floor(end.x / TILE_SIZE) == seg->until.x / 64)
