...
/*   Updated: 2024/05/17 11:45:27 by mbruyant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	ft_check_diag_wall(t_data *cub, t_point_d *end)
{
	...
	(int) end->y % TILE_SIZE == TILE_SIZE - 1) && \
	((cub->map->raw_map[(int) end->y / TILE_SIZE]\
	[(int) (end->x - 1) / TILE_SIZE] && \
	cub->map->raw_map[(int) end->y / TILE_SIZE]\
	[(int) (end->x - 1) / TILE_SIZE] == '1') || \
	(cub->map->raw_map[(int) end->y / TILE_SIZE]\
	[(int) (end->x + 1) / TILE_SIZE] && \
	cub->map->raw_map[(int) end->y / TILE_SIZE]\
	[(int) (end->x + 1) / TILE_SIZE] == '1')))
